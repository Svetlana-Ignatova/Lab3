...
#ifndef S_LAB3_BINARYTREE_H
#define S_LAB3_BINARYTREE_H
...
public:
    ...
    void CreateVector(Node<T> *root, vector<Node<T>*> &nodes);
...
///балансировка
template<typename T>
void BinaryTree<T>::CreateVector(Node<T>* r, vector<Node<T>*> &nodes)
{
    if(r == nullptr)
        return;
    CreateVector(r->left, nodes);
    nodes.push_back(r);
    CreateVector(r->right, nodes);
}
...
{
    vector<Node<T>*> nodes;
    CreateVector(r, nodes);
    int n = nodes.size();
    return BuildTree(nodes, 0, n - 1);
}


#endif //S_LAB3_BINARYTREE_H
